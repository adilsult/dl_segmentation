"""compare_models.py — Train and compare all model configurations on BraTS data.

Trains hybrid model configs (lighter transformer for CPU) and loads prior
results from unet_dicebce and attn_dicebce runs to produce a full comparison table.

Run with: python compare_models.py
Results saved to: runs/comparison_summary.json
"""
...
# Configs to train in this script (unet and attn results loaded from prior runs)
CONFIGS = [
    {"name": "hybrid_dicebce",       "model": "hybrid", "loss": "dicebce"},
    {"name": "hybrid_focal_tversky", "model": "hybrid", "loss": "focal_tversky"},
]

# Shared hyperparameters for all configs
EPOCHS       = 30
BATCH_SIZE   = 4
VAL_RATIO    = 0.2
SEED         = 42
...
MIN_DELTA    = 0.001   # minimum val_dice improvement to reset patience
...
def build_model(name: str) -> torch.nn.Module:
    ...
    if name == "unet":
        ...
        # Lighter transformer for CPU: 2 layers, 4 heads, smaller FFN
...
def main() -> None:
    ...
    # Discover all BraTS cases in the current directory
    case_paths = discover_case_paths(".", max_cases=0)
    print(f"Cases: {len(case_paths)}")
    for p in case_paths:
        print(f"  {p}")

    # Build combined dataset from all cases (slice-level split)
    ...
    # Shared train/val loaders — same split for all model configs (fair comparison)
    ...
    # Load existing results from prior unet and attention_unet runs
